...
def dashboard(request):
    ...
    return render(
        request,
        "jobright/dashboard.html",
        {
            "state": state,
            "job_count": JobrightJob.objects.count(),
...
def jobs_api(request):
    ...
    qs = JobrightJob.objects.all().order_by("-created_at")
...
def status_api(request):
    ...
    elif status == JobrightScraperState.STATUS_RUNNING:
        status = JobrightScraperState.STATUS_STOPPED
    total_kw = len(settings.KEYWORDS)
    kw_done = state.keyword_index
    if status == JobrightScraperState.STATUS_IDLE and kw_done >= total_kw:
        kw_done = total_kw
    elif running:
