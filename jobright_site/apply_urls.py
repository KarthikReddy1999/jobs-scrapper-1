...
def is_specific_job_apply_url(url):
    """Reject ATS board homepages and generic career landing pages (not a single job)."""
    if not url:
        return False
    low = url.lower()
    ...
    if any(
        ...
        return False
    if ("youtube.com" in low or "google.com" in low) and "/jobs/results/" not in low:
        if not re.search(r"/jobs/\d", low):
            return False
